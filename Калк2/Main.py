...
def rational_two(update, context):
    user = update.message.from_user
    logger.info("Вы ввели число: %s: %s", user.first_name, update.message.text)
    get_rational = update.message.text
    if get_rational.isdigit():
        get_rational = float(get_rational)
...
def operatons_rational(update, context):
    user = update.message.from_user
    logger.info(
        "Вы выбрали операцию %s: %s", user.first_name, update.message.text)
...
def complex_one(update, context):
    user = update.message.from_user
    logger.info(
        "Вы ввели число %s: %s", user.first_name, update.message.text)
    user_choice = update.message.text
...
def cancel(update, _):
    user = update.message.from_user
    logger.info("Вы %s отменили разговор.", user.first_name)
    update.message.reply_text('Спасибо, до свидания!')
    return ConversationHandler.END
